...
var flatten = function(options) {
    ...
    files.forEach(function(filename) {
        ...
        if (fs.lstatSync(licenseFile).isFile()) {
            ...
            var filePath = moduleInfo.licenseFile;
            if (filePath && filePath.indexOf('node_modules/') > 0) {
              var index = filePath.lastIndexOf('node_modules/') + 'node_modules/'.length;
              var relativePath = filePath.substring(index);
              var repoNameEndIndex = relativePath.indexOf('/');
              if (repoNameEndIndex > 0) {
                relativePath = relativePath.substring(repoNameEndIndex);
              }
              moduleInfo.licenseFile = moduleInfo.repository + '/blob/master' + relativePath;
            }
        }
    });
...
exports.init = function(options, callback) {
    ...
    asyncMap(paths, function(path, cb) {
        read(path, opts, function(err, json) {
        var data = flatten({
                deps: json,
                data: {},
                color: options.color,
    ...
    var colorize = options.color,
            ...
            inputError = null;
    function readCompleted(err, results) {
      var data = results.reduce(function(prev, cur) {
